...
class CustomEnv(gym.Env, EzPickle):
    metadata = {
    ...
    def step(self, action):
        ...
        reward2 = 0
        shaping1 = \
            - 100*np.sqrt(state[0]*state[0] + state[1]*state[1])  # And ten points for legs contact, the idea is if you
        ...
        reward1 -= m_power*0.30  # less fuel spent is better, about -30 for heuristic landing
        reward1 -= s_power*0.03

        reward2 -= m_power*0.30  # less fuel spent is better, about -30 for heuristic landing
        reward2 -= s_power*0.03
        ...
        if not self.lander.awake:
            ...
            reward1 = +100
            reward2 = +100
        return np.array(state, dtype=np.float32), reward1,reward2, done, {}
